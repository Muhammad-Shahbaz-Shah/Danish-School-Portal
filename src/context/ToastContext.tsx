...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addToast = useCallback(
    (
      ...
      const id =
        customId ||
        "toast-" + Date.now() + "-" + Math.random().toString(36).substr(2, 4);

      setToasts((prev) => {
        const filtered = prev.filter((t) => t.id !== id);
        return [
          ...filtered.slice(-4),
          { id, type, title, message, isPermanent, onRetry, retryText },
        ];
      });
  ...
  const reconnectDb = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (data.success) {
        setDbConnected(true);
        setToasts((prev) =>
          prev.filter((t) => t.id !== "toast-db-status-warning"),
        );
        addToast(
          "MongoDB Atlas Connected!",
          "success",
          data.message ||
            `Successfully connected to MongoDB database "${data.dbName}".`,
        );
        return true;
      } else {
        addToast(
          ...
          "error",
          data.error ||
            "Could not connect to MongoDB Atlas cluster. Check your URI or network.",
        );
        return false;
      }
    } catch (e: any) {
      addToast("Connection Error", "error", e);
      return false;
    }
  }, [addToast]);
          ...
          setToasts((prev) => {
            if (prev.some((t) => t.id === "toast-db-status-warning")) {
              addToast(
                "Daatabase Active",
                "success",
                `Connected to database "${data.dbName || "daanish_schools_db"}".`,
              );
            }
      ...
      if (res.ok || res.status < 500) {
        if (isOffline) {
          ...
          addToast(
            "Internet Restored",
            "success",
            "You are back online. All services are connected.",
          );
        }
        return true;
      }
      ...
      setToasts((prev) => {
        if (prev.some((t) => t.id === "toast-db-status-warning")) {
          addToast(
            "Daatabase Active",
            "success",
            `Connected to database daanish_schools_db`,
          );
        }
        return prev.filter((t) => t.id !== "toast-db-status-warning");
      });
...
const ToastCardItem: React.FC<{ toast: ToastItem; onRemove: () => void }> = ({
  toast,
  onRemove,
}) => {
  ...
  if (toast.type === "success") {
    ...
    bgColors =
      "bg-slate-900 border-amber-500/40 text-white shadow-amber-950/30 ring-1 ring-amber-500/20";
            ...
            <div className="mt-2.5 flex items-center gap-2">
              ...
              >
                ...
                <span>
                  {isRetrying
                    ? "Connecting..."
                    : toast.retryText || "Retry Connection"}
                </span>
              </button>
            </div>
          )}
